...
public class GroupChatRoom extends AppCompatActivity {
   ...
   // private DatabaseReference mDatabase;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Set up ListView
        final ListView listView = (ListView) findViewById(R.id.listViewGroupMessagesSend);
        final ArrayAdapter<String> adapter = new ArrayAdapter<>(GroupChatRoom.this, android.R.layout.simple_list_item_1, android.R.id.text1);
        listView.setAdapter(adapter);

        // Add items via the Button and EditText at the bottom of the view.
        final EditText text = (EditText) findViewById(R.id.etGetGroupMessages);
        final Button button = (Button) findViewById(R.id.bSendGroupMessages);
        final String email="";

        //getEmailfrom signup

         name=username(username(mFirebaseUser.toString()));

        Intent i = getIntent();
        //The second parameter below is the default string returned if the value is not there.
        //userName = i.getExtras().getString("userName");
        choosenContact=i.getExtras().getString("GroupName");
        //mDatabase.child("users").child("GroupNames").child(choosenContact).push().child("title").setValue(" ");
        ...
        button.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                ...
                mDatabase.child("users").child("GroupNames").child(choosenContact).child("messages").push().child("title").setValue("["+name+"]"+text.getText().toString());
                text.setText("");
            }
        });
        ...
        mDatabase.child("users").child("GroupNames").child(choosenContact).child("messages").addChildEventListener(new ChildEventListener() {
...
/*
public class SendMessages1 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        button.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                ...
                //mDatabase.child("users").child(mUserId).child("items").push().child("title").setValue(text.getText().toString());
                //mDatabase.child("users").child(userName).child("messages").push().child("title").setValue(text.getText().toString());
                mDatabase.child("users").child(ContactMessages).child("messages").push().child("title").setValue(text.getText().toString());
                text.setText("");
            }
        });
